 ...
 * {
 ...
 .code-box { background: var(--surface-2); border: 1px solid var(--rule); border-radius: 2px; padding: 16px; font-family: var(--mono); font-size: 12px; overflow-x: auto; color: var(--ink); position: relative; }
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 // Injeção automática de botão ÚNICO de cópia em cada bloco de código ou prompt
 const targets = [];
 document.querySelectorAll('.code-box').forEach(box => targets.push(box));
 document.querySelectorAll('pre').forEach(pre => {
  if (!pre.closest('.code-box')) {
   targets.push(pre);
  }
 });
 ...
 targets.forEach((block) => {
  ...
  btn.setAttribute('aria-label', 'Copiar código ou prompt');
  
  btn.addEventListener('click', async (e) => {
   e.stopPropagation();
   let textToCopy = '';
   const codeElem = block.querySelector('pre') || block.querySelector('code');
   if (codeElem) {
    textToCopy = codeElem.innerText.replace(/Copiar/g, '').trim();
   } else {
    textToCopy = block.innerText.replace(/Copiar/g, '').trim();
   }
...
def gerar_markdown_ecossistema(dados: dict) -> str:
  ...
  linhas.extend([
    f"",
    f"### Alocação Técnica de Recursos por Serviço (vCPU & RAM)",
  ...
  linhas.extend([
    ...
    f"- ** Gateway de Borda & Ingress TLS:** {integracao.get('gateway_reverse_proxy')}",
    f"",
  ...
  if vps_spec:
    linhas.extend([
      f"",
      f"### Especificação da VPS Ideal para o Ecossistema Completo (e Por Que)",
  ...
  for mig in dados.get("guia_migracao_dados", []):
    ...
    for p in mig.get("passos", []):
      linhas.append(f" 1. {p}")
    linhas.extend([
      f"- ** Cuidados Críticos:** {mig.get('cuidados')}",
      f""
    ])
       ...
       <div class="entry-body">
        ...
        <div class="entry-section">
         ...
         <div class="infra-grid">
          ...
          <div class="infra-card">
           <span class="infra-lbl"> CPU Recomendada</span>
  ...
  migracao_cards_html = "".join([
    ...
    <div class="integ-card" style="margin-bottom: 14px;">
     ...
     <div style="background: var(--flag-soft); border-left: 3px solid var(--flag); padding: 8px 12px; border-radius: 2px; font-size: 12.5px; color: var(--flag); margin-top: 6px;">
      <strong> Cuidado Essencial:</strong> {mig.get('cuidados')}
     </div>
    </div>
    """
    for mig in dados.get("guia_migracao_dados", [])
...
<div class="wrap">
 ...
 <div class="racional-box" style="margin-bottom: 20px;">
  <p><strong> Configuração Consolidada Recomendada:</strong> <code>{vps_spec.get('perfil_recomendado', '8 vCPU Dedicated / 16 GB RAM ECC / 160-240 GB NVMe SSD / 1 Gbps / Ubuntu 24.04 LTS')}</code></p>
 ...
 <h3 style="font-family: var(--font-serif); font-size: 20px; margin: 24px 0 10px; color: var(--ink);"> Alocação Técnica de Recursos por Contêiner (vCPU &amp; Memória RAM)</h3>
 ...
 <div class="racional-box" style="margin-bottom: 20px;">
  <p><strong> Segurança e Topologia de Rede:</strong> {deploy.get('arquitetura_rede_seguranca')}</p>
 </div>
 ...
 <div class="racional-box">
  <p><strong> Configuração Recomendada:</strong> {vps_spec.get('perfil_recomendado')}</p>
 ...
 <div class="racional-box">
  <p><strong> Regra de Ouro do Backup (3-2-1):</strong> {seg.get('arquitetura_backup_321')}</p>
 </div>
...
def compilar_ecossistema_tripartite(slug: str):
  ...
  try:
    ...
    print(f"  Typst não encontrado ou falha no PDF: {e}")
  ...
  deploy = dados.get("deploy_consolidado", {})
  body_deploy = f"""
  <div class="racional-box"><p><strong> Rede &amp; Segurança:</strong> {deploy.get('arquitetura_rede_seguranca')}</p></div>
  ...
  seg = dados.get("seguranca_backup_lgpd", {})
  body_seg = f"""
  <div class="racional-box"><p><strong> Política 3-2-1:</strong> {seg.get('arquitetura_backup_321')}</p></div>
...
"""
  ...
  try:
    ...
    print(f" Erro ao registrar ecossistema no SQLite: {e}")
