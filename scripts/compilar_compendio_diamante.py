...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Compilador Diamante R5 de Listas Horizontais (list-*.html)")
    parser.add_argument("arquivo", help="Caminho do arquivo JSON com dados estruturados da lista")
    parser.add_argument("--slug", help="Slug curto (gera list-<slug>.html). Se omitido, usa o slug do JSON.")
    ...
    slug_final = args.slug or dados.get("slug", caminho.stem.replace("dossie-", "").replace("lista-", ""))
    if not slug_final.startswith("list-") and not slug_final.startswith("tco-") and not slug_final.startswith("guia-"):
        nome_arquivo = f"list-{slug_final}.html"
    else:
        nome_arquivo = f"{slug_final}.html" if not slug_final.endswith(".html") else slug_final

    html_gerado = compilar_compendio_diamante(dados)
    ...
    print(f"✅ Lista Horizontal compilada com sucesso!")
