...
- **O que você verá na tela:** Mensagem 'Firewall is active and enabled on system startup'.
...
- **O que você verá na tela:** Instalação dos pacotes Docker e registro do serviço no sistema operacional.
...
- **O que você verá na tela:** Pastas criadas em menos de um segundo.
...
- **O que você verá na tela:** Docker inicia o container whisperx-engine.
...
- **O que você verá na tela:** Barras de progresso de transcrição e detecção de locutores.
...
- **O que você verá na tela:** Linha de agendamento salva na tabela do cron.
...
- ** Sintoma:** Erro 'CUDA out of memory' durante a diarização
...
> **Princípio de Isolamento:** O WhisperX opera em ambiente virtual Python dedicado (/opt/whisperx-venv). Sua remoção limpa modelos cacheados sem alterar a versão do Python do sistema nem pacotes de outras aplicações.
...
- **Alerta de Segurança:** Não encerre o driver NVIDIA nem mate processos globais da GPU.
...
- **Alerta de Segurança:** A exclusão atinge apenas a pasta /opt/whisperx-venv. Nenhuma biblioteca do sistema é afetada.
...
- **Alerta de Segurança:** Se outro microsserviço utilizar a porta 8000, certifique-se de fechar apenas se exclusivo.
...
- **Alerta de Segurança:** Não remova outros arquivos em /etc/systemd/system/.
