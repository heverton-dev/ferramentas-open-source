...
- **O que você verá na tela:** Status do firewall ativo com portas 22, 80, 443 liberadas.
...
- **O que você verá na tela:** NVIDIA toolkit instalado e verificado via nvidia-smi.
...
- **O que você verá na tela:** Download dos pesos do modelo XTTS-v2 (aprox. 1.8 GB) e inicializacao do servidor HTTP na porta 5002.
...
- **O que você verá na tela:** Nginx configurado com proxy_pass para http://127.0.0.1:5002.
...
- ** Sintoma:** CUDA out of memory (OOM)
 - **Causa:** Lotes de texto excessivamente longos sintetizados simultaneamente.
- ** Sintoma:** Voz clonada soando robotica ou com eco
...
> **Princípio de Isolamento:** A desinstalacao remove exclusivamente as chaves de API do ElevenLabs, scripts legados de download em nuvem e bibliotecas proprietarias, mantendo a VPS limpa e o servidor local XTTS totalmente isolado sem consumo residual de GPU.
...
- **Alerta de Segurança:** Apenas audite as ocorrencias antes de remover qualquer arquivo.
...
- **Alerta de Segurança:** Mantenha as amostras de audio originais guardadas em armazenamento seguro.
...
- **Alerta de Segurança:** Valide a geracao de audio local antes de revogar o token da nuvem.
...
- **Alerta de Segurança:** Garante que nenhuma requisicao sera enviada para os servidores da ElevenLabs.
...
- **Alerta de Segurança:** Confirme que todos os projetos em producao estao atendidos pelo XTTS local.
