...
- **O que você verá na tela:** Na Hetzner, você verá um painel com o IP do servidor. No seu registrador de domínio, você alterará os nameservers.
...
- **O que você verá na tela:** Será solicitada a senha root (ou chave SSH se configurado). A tela mostrará a atualização de pacotes por alguns minutos.
...
- **O que você verá na tela:** Git clonar o repositório para /opt/mailcow-dockerized durante 30 a 60 segundos.
...
- **O que você verá na tela:** O script perguntará em sequência: 'Hostname FQDN?', 'Timezone?', 'Full name of Organization?'. Depois criará os certificados SSL automaticamente.
...
- **O que você verá na tela:** Será exibido 'Pulling' de múltiplas imagens, depois 'Creating', 'Starting' de cada container. Ao final, 'docker compose ps' mostrará 15+ containers em status 'Up'.
...
- **O que você verá na tela:** O navegador mostrará um painel de controle com abas 'Mail', 'Mailboxes', 'Domains', 'DNS' e 'Monitoring'.
...
- **O que você verá na tela:** Na interface Mailcow, verá caixas com os registros DNS. No registrador de domínio, verá abas 'DNS', 'Registros MX', 'Registros TXT'.
...
- **O que você verá na tela:** No cliente de email, você configurará IMAP (porta 993) e SMTP (porta 587 ou 465) com seu usuário. Emails chegarão normalmente.
...
> **Princípio de Isolamento:** A desinstalação remove exclusivamente os containers, volumes e dados do Mailcow, preservando intactos o Docker, Nginx reverso, PostgreSQL e outros projetos hospedados na VPS.
...
- **Alerta de Segurança:** NÃO DELETE ESTES ARQUIVOS. Guarde em local seguro ou em outro servidor.
...
- **Alerta de Segurança:** Use 'down' nunca 'down -v' (o -v removeria volumes). Dados ainda existem.
...
- **Alerta de Segurança:** Após remover volumes, dados são definitivamente apagados. Confirme que backup foi feito.
...
- **Alerta de Segurança:** Mantenha firewall ativo. Verifique com 'ufw status' antes e depois.
...
- **Alerta de Segurança:** Backup já foi feito no Passo 1. Se alterou de ideia, ainda há tempo. Após 'rm -rf', dados são irrecuperáveis.
...
- **Alerta de Segurança:** Propag ação DNS leva até 48 horas. Emails podem ficar presos em fila durante transição. Mantenha backup do Passo 1 acessível.
