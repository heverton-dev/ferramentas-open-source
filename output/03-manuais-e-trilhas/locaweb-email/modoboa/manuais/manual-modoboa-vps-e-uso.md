...
- 🖥️ **O que você verá na tela:** Um email chega com 'Seu servidor foi criado com sucesso. IP: XXX.XXX.XXX.XXX'.
...
- 🖥️ **O que você verá na tela:** O sistema pedirá senha e confirmará 'Firewall is active and enabled on system startup'.
...
- 🖥️ **O que você verá na tela:** Linhas de download e compilação passam pela tela durante 2 a 3 minutos.
...
- 🖥️ **O que você verá na tela:** Docker fará pull das imagens e exibirá 'Container modoboa-web Started', 'Container modoboa-postgres Started', 'Container modoboa-redis Started'.
...
- 🖥️ **O que você verá na tela:** Certbot verificará seu domínio (deve estar apontando para o IP do servidor) e confirmará 'Congratulations! Your certificate is issued'.
...
> 🛡️ **Princípio de Isolamento:** Remove exclusivamente containers Modoboa preservando Docker daemon e outros serviços.
...
- ⚠️ **Alerta de Segurança:** Não execute 'docker system prune -a'.
...
- ⚠️ **Alerta de Segurança:** BACKUP ANTES: docker compose exec postgres pg_dump -U modoboa modoboa > backup.sql
...
- ⚠️ **Alerta de Segurança:** Mantém SSH e web abertos.
