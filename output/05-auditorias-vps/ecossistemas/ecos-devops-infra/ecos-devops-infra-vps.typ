...
#table(
  ...
  [Nocodb Service],
  [Componente da Stack Ecossistema DevOps & Engenharia de Dados (NocoDB + Supabase + n8n + Directus)],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ecos-devops-infra_*`).
...
#table(
  ...
  [`nocodb.vpsconexao.org`],
  [A],
  [IP da VPS],
  [DNS Only (Nuvem Cinza inicial)],
  [`supabase.vpsconexao.org`],
  [A],
  [IP da VPS],
  [DNS Only (Nuvem Cinza inicial)],
  [`directus.vpsconexao.org`],
  [A],
  [IP da VPS],
  [DNS Only (Nuvem Cinza inicial)],
)
...
+ No campo **Name**, digite exatamente: `ecos-devops-infra`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Ecossistema DevOps & Engenharia de Dados (NocoDB + Supabase + n8n + Directus) - App Principal`.
+ **URL:** `https://nocodb.vpsconexao.org`.
...
+ Localize a stack `ecos-devops-infra` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `ecos-devops-infra` foi removida.
