...
A VPS de producao possui *12 vCPUs* e *47.05 GB de memoria RAM*, operando com folga substancial (*~43.99 GB de memoria livre*). A incorporacao do alvo demanda *4 vCPUs* e *6.0 GB de RAM*, preservando a estabilidade das aplicacoes existentes (Mautic, Evolution, n8n).
...
2. *Namespace de Volumes Isolados:* Todos os dados persistentes sao armazenados em volumes Docker dedicados (`ecos-devops-infra_*`), impedindo sobrescrita de bancos de dados legados.
...
#table(
  ...
  [Supabase Service], [https://supabase.vpsconexao.org], [Traefik SNI], [Rede network_conexao],
  [Directus Service], [https://directus.vpsconexao.org], [Traefik SNI], [Rede network_conexao],
...
#table(
  ...
  [supabase.vpsconexao.org], [A], [IP da VPS], [DNS Only],
  [directus.vpsconexao.org], [A], [IP da VPS], [DNS Only],
...
2. Navegue em *Stacks* > *+ Add stack* e defina o nome `ecos-devops-infra`.
...
1. *Via Portainer:* Selecione a stack `ecos-devops-infra` e clique em *Delete this stack*.
2. *Via Terminal SSH:*
```bash
