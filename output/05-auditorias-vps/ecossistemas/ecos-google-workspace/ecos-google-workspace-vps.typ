...
#table(
  ...
  [Drive Service],
  [Componente da Stack Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad)],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ecos-google-workspace_*`).
...
+ No campo **Name**, digite exatamente: `ecos-google-workspace`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad) - App Principal`.
+ **URL:** `https://drive.vpsconexao.org`.
...
+ Localize a stack `ecos-google-workspace` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `ecos-google-workspace` foi removida.
