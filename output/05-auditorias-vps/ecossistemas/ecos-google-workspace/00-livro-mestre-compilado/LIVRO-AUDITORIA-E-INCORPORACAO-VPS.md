...
**Alvo de Incorporação:** Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad)  
**Data da Auditoria:** 28/08/2026  
**Veredito Técnico:** **TOTALMENTE VIAVEL (100% HOMOLOGADO)** (Score: 100/100)  
...
| **Memória RAM Global** | 47.05 GB | ~3.06 GB | **~43.99 GB** | 7.0 GB | APROVADO |
...
**Alvo:** Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad) | **Data:** 28/08/2026
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ecos-google-workspace_*`).
...
| `office.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `mail.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `docs.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `sso.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
...
- Registro A: `office.vpsconexao.org` -> IP da VPS
- Registro A: `mail.vpsconexao.org` -> IP da VPS
- Registro A: `docs.vpsconexao.org` -> IP da VPS
- Registro A: `sso.vpsconexao.org` -> IP da VPS
...
5. No campo **Name**, digite exatamente: `ecos-google-workspace`.
...
- `https://office.vpsconexao.org`
- `https://mail.vpsconexao.org`
- `https://docs.vpsconexao.org`
- `https://sso.vpsconexao.org`
...
2. **Nome:** `Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad) - App Principal`.
...
Todos os recursos criados para o alvo `Ecossistema Google Workspace (Nextcloud + ONLYOFFICE + Stalwart + CryptPad)` foram encapsulados no namespace `ecos-google-workspace`.
...
3. Localize a stack `ecos-google-workspace` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=ecos-google-workspace -q | xargs -r docker volume rm
```
...
2. `docker stack ls` -> Confirme que a stack `ecos-google-workspace` foi removida.
