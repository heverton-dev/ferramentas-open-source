...
A incorporacao da stack `ecos-crm-marketing` demanda **3 vCPUs** e **4.5 GB de RAM**, mantendo margem de seguranca estrita.

| Metrica de Infraestrutura | Capacidade Total | Ocupacao Atual (Est.) | Demanda da Stack | Headroom Restante | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Processamento (vCPU)** | 12 vCPUs | ~1.5 vCPUs | 3 vCPUs | **~7.3 vCPUs Livres** | APROVADO |
| **Memoria RAM Global** | 47.05 GB | ~3.06 GB | 4.5 GB | **~39.5 GB Livres** | APROVADO |
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ecos-crm-marketing_*`).
...
| :--- | :--- | :--- | :--- |
| **Chat Service** | `https://chat.vpsconexao.org` | Roteamento Traefik SNI | Ativo na Rede `network_conexao` |
| **Crm Service** | `https://crm.vpsconexao.org` | Roteamento Traefik SNI | Ativo na Rede `network_conexao` |
| **Wpp Service** | `https://wpp.vpsconexao.org` | Roteamento Traefik SNI | Ativo na Rede `network_conexao` |
| **Campaigns Service** | `https://campaigns.vpsconexao.org` | Roteamento Traefik SNI | Ativo na Rede `network_conexao` |
...
| `crm.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `wpp.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `campaigns.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
...
services:
  ecos-crm-marketing_app:
    image: ecos-crm-marketing:latest
    networks:
      - network_conexao
    volumes:
      - ecos-crm-marketing_data:/data
    deploy:
      mode: replicated
      replicas: 1
      labels:
        ...
        - "traefik.http.routers.ecos-crm-marketing.rule=Host(`chat.vpsconexao.org`)"
        - "traefik.http.routers.ecos-crm-marketing.entrypoints=websecure"
        - "traefik.http.routers.ecos-crm-marketing.tls=true"
        - "traefik.http.routers.ecos-crm-marketing.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.ecos-crm-marketing.loadbalancer.server.port=80"
        - "traefik.http.services.ecos-crm-marketing.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '3'
          memory: 4608M
...
3. Nomeie a stack como: `ecos-crm-marketing`.
...
2. **URL:** `https://chat.vpsconexao.org`.
...
### Via Portainer:
1. Acesse **Stacks**, selecione `ecos-crm-marketing` e clique em **Delete this stack**.

### Via Terminal SSH:
```bash
docker stack rm ecos-crm-marketing
```
...
2. `docker stack ls` -> Confirmar ausencia da stack `ecos-crm-marketing`.
