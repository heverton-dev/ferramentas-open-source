...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Ecossistema CRM & Automacao de Marketing (Chatwoot + Twenty + Evolution + Mautic)]\n
      #v(0.4em)
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Ecossistema CRM & Automacao de Marketing (Chatwoot + Twenty + Evolution + Mautic)
...
#table(
  ...
  [Chat Service],
  [Componente da Stack Ecossistema CRM & Automacao de Marketing (Chatwoot + Twenty + Evolution + Mautic)],
...
**Alvo:** Ecossistema CRM & Automacao de Marketing (Chatwoot + Twenty + Evolution + Mautic) | **Data:** 28/08/2026
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ecos-crm-marketing_*`).
...
+ No campo **Name**, digite exatamente: `ecos-crm-marketing`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Ecossistema CRM & Automacao de Marketing (Chatwoot + Twenty + Evolution + Mautic) - App Principal`.
+ **URL:** `https://chat.vpsconexao.org`.
...
+ Localize a stack `ecos-crm-marketing` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=ecos-crm-marketing -q | xargs -r docker volume rm
```
...
+ `docker stack ls` -> Confirme que a stack `ecos-crm-marketing` foi removida.
