...
A VPS de producao possui *12 vCPUs* e *47.05 GB de memoria RAM*, operando com folga substancial (*~43.99 GB de memoria livre*). A incorporacao do alvo demanda *3 vCPUs* e *4.5 GB de RAM*, preservando a estabilidade das aplicacoes existentes (Mautic, Evolution, n8n).
...
#table(
  ...
  [Processamento], [12 vCPUs], [~1.5 vCPUs], [3 vCPUs], [APROVADO],
  [Memoria RAM], [47.05 GB], [~3.06 GB], [4.5 GB], [APROVADO],
...
2. *Namespace de Volumes Isolados:* Todos os dados persistentes sao armazenados em volumes Docker dedicados (`ecos-crm-marketing_*`), impedindo sobrescrita de bancos de dados legados.
...
2. Navegue em *Stacks* > *+ Add stack* e defina o nome `ecos-crm-marketing`.
...
- *Tipo:* HTTP(s) Monitor.
- *URL Principal:* `https://chat.vpsconexao.org`.
...
1. *Via Portainer:* Selecione a stack `ecos-crm-marketing` e clique em *Delete this stack*.
2. *Via Terminal SSH:*
```bash
