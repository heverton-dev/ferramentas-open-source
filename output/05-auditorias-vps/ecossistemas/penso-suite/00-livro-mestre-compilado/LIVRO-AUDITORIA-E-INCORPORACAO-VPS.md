...
**Alvo de Incorporação:** Ecossistema penso-suite  
**Data da Auditoria:** 28/08/2026  
**Veredito Técnico:** **TOTALMENTE VIAVEL (100% HOMOLOGADO)** (Score: 100/100)  
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Processamento (vCPU)** | 12 vCPUs | ~1.5 vCPUs | **~10.3 vCPUs** | 3.0 vCPUs | APROVADO |
| **Memória RAM Global** | 47.05 GB | ~3.06 GB | **~43.99 GB** | 5.0 GB | APROVADO |
...
- Memoria RAM abundante: 43.99 GB livres para suportar a carga de 5.0 GB com alta folga.
...
| :--- | :--- | :--- | :--- |
| **App Service** | Componente da Stack Ecossistema penso-suite | `https://app.vpsconexao.org` | Roteamento Traefik SNI |
| **Api Service** | Componente da Stack Ecossistema penso-suite | `https://api.vpsconexao.org` | Roteamento Traefik SNI |
...
**Alvo:** Ecossistema penso-suite | **Data:** 28/08/2026
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `penso-suite_*`).
...
services:
  penso-suite_app:
    image: penso-suite:latest
    networks:
      - network_conexao
    volumes:
      - penso-suite_data:/data
    deploy:
      mode: replicated
      replicas: 1
      labels:
        ...
        - "traefik.http.routers.penso-suite.rule=Host(`app.vpsconexao.org`)"
        - "traefik.http.routers.penso-suite.entrypoints=websecure"
        - "traefik.http.routers.penso-suite.tls=true"
        - "traefik.http.routers.penso-suite.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.penso-suite.loadbalancer.server.port=80"
        - "traefik.http.services.penso-suite.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '3.0'
          memory: 5120M
...
| :--- | :--- | :--- | :--- |
| `app.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
| `api.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
...
- Registro A: `app.vpsconexao.org` -> IP da VPS
- Registro A: `api.vpsconexao.org` -> IP da VPS
...
5. No campo **Name**, digite exatamente: `penso-suite`.
...
- `https://app.vpsconexao.org`
- `https://api.vpsconexao.org`
...
2. **Nome:** `Ecossistema penso-suite - App Principal`.
...
Todos os recursos criados para o alvo `Ecossistema penso-suite` foram encapsulados no namespace `penso-suite`.
...
3. Localize a stack `penso-suite` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=penso-suite -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=penso-suite_ -q | xargs -r docker volume rm
```
...
2. `docker stack ls` -> Confirme que a stack `penso-suite` foi removida.
