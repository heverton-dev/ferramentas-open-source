...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Ecossistema penso-suite]\n
      #v(0.4em)
...
#table(
  ...
  [App Service],
  [Componente da Stack Ecossistema penso-suite],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `penso-suite_*`).
...
+ No campo **Name**, digite exatamente: `penso-suite`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Ecossistema penso-suite - App Principal`.
+ **URL:** `https://app.vpsconexao.org`.
...
+ Localize a stack `penso-suite` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `penso-suite` foi removida.
