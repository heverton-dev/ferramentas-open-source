...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Ferramenta coqui-xtts]\n
      #v(0.4em)
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Ferramenta coqui-xtts
...
#table(
  ...
  [Coqui-xtts Service],
  [Componente da Stack Ferramenta coqui-xtts],
  [`https://coqui-xtts.vpsconexao.org`],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `coqui-xtts_*`).
...
services:
  coqui-xtts_app:
    image: coqui-xtts:latest
    networks:
      - network_conexao
    volumes:
      - coqui-xtts_data:/data
    deploy:
      mode: replicated
      replicas: 1
      labels:
        ...
        - "traefik.http.routers.coqui-xtts.rule=Host(`coqui-xtts.vpsconexao.org`)"
        - "traefik.http.routers.coqui-xtts.entrypoints=websecure"
        - "traefik.http.routers.coqui-xtts.tls=true"
        - "traefik.http.routers.coqui-xtts.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.coqui-xtts.loadbalancer.server.port=80"
        - "traefik.http.services.coqui-xtts.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '1.5'
          memory: 1536M
...
- Registro A: `coqui-xtts.vpsconexao.org` -> IP da VPS
...
+ No campo **Name**, digite exatamente: `coqui-xtts`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Ferramenta coqui-xtts - App Principal`.
+ **URL:** `https://coqui-xtts.vpsconexao.org`.
...
Todos os recursos criados para o alvo `Ferramenta coqui-xtts` foram encapsulados no namespace `coqui-xtts`.
...
+ Localize a stack `coqui-xtts` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=coqui-xtts -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=coqui-xtts_ -q | xargs -r docker volume rm
```
...
+ `docker stack ls` -> Confirme que a stack `coqui-xtts` foi removida.
