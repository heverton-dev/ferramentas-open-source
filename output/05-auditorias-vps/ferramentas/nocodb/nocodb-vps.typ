...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `nocodb_*`).
...
+ No campo **Name**, digite exatamente: `nocodb`.
...
+ Localize a stack `nocodb` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `nocodb` foi removida.
