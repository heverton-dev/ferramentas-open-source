...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[NocoDB]\n
      #v(0.4em)
...
A VPS de producao possui *12 vCPUs* e *47.05 GB de memoria RAM*, operando com folga substancial (*~43.99 GB de memoria livre*). A incorporacao do alvo demanda *1.0 vCPUs* e *1.0 GB de RAM*, preservando a estabilidade das aplicacoes existentes (Mautic, Evolution, n8n).
...
#table(
  ...
  [Processamento], [12 vCPUs], [~1.5 vCPUs], [1.0 vCPUs], [APROVADO],
  [Memoria RAM], [47.05 GB], [~3.06 GB], [1.0 GB], [APROVADO],
...
2. *Namespace de Volumes Isolados:* Todos os dados persistentes sao armazenados em volumes Docker dedicados (`nocodb_*`), impedindo sobrescrita de bancos de dados legados.
...
#table(
  ...
  [Nocodb Service], [https://nocodb.vpsconexao.org], [Traefik SNI], [Rede network_conexao],
...
#table(
  ...
  [nocodb.vpsconexao.org], [A], [IP da VPS], [DNS Only],
...
2. Navegue em *Stacks* > *+ Add stack* e defina o nome `nocodb`.
  ...
  nocodb_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.nocodb.rule=Host(`nocodb.vpsconexao.org`)"
        - "traefik.http.routers.nocodb.entrypoints=websecure"
        - "traefik.http.routers.nocodb.tls=true"
        - "traefik.http.routers.nocodb.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.nocodb.loadbalancer.server.port=3000"
...
- *Tipo:* HTTP(s) Monitor.
- *URL Principal:* `https://nocodb.vpsconexao.org`.
...
1. *Via Portainer:* Selecione a stack `nocodb` e clique em *Delete this stack*.
2. *Via Terminal SSH:*
```bash
...
```bash
docker volume ls --filter name=nocodb_ -q | xargs -r docker volume rm
```
