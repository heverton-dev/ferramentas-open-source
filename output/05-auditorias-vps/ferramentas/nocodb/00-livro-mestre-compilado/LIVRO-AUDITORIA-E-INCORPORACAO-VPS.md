...
| **Memória RAM Global** | 47.05 GB | ~3.06 GB | **~43.99 GB** | 1.0 GB | APROVADO |
...
| :--- | :--- | :--- | :--- |
| **Nocodb Service** | Componente da Stack NocoDB | `https://nocodb.vpsconexao.org` | Roteamento Traefik SNI |
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `nocodb_*`).
...
5. No campo **Name**, digite exatamente: `nocodb`.
...
Todos os recursos criados para o alvo `NocoDB` foram encapsulados no namespace `nocodb`.
...
3. Localize a stack `nocodb` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=nocodb -q | xargs -r docker volume rm
```
...
2. `docker stack ls` -> Confirme que a stack `nocodb` foi removida.
