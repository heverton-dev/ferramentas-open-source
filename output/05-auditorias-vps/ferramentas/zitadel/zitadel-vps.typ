...
#table(
  ...
  [Sso Service],
  [Componente da Stack Zitadel Identity Management],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `zitadel_*`).
...
+ No campo **Name**, digite exatamente: `zitadel`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Zitadel Identity Management - App Principal`.
+ **URL:** `https://sso.vpsconexao.org`.
...
+ Localize a stack `zitadel` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `zitadel` foi removida.
