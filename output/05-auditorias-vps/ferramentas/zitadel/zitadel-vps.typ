...
A VPS de producao possui *12 vCPUs* e *47.05 GB de memoria RAM*, operando com folga substancial (*~43.99 GB de memoria livre*). A incorporacao do alvo demanda *1.5 vCPUs* e *1.5 GB de RAM*, preservando a estabilidade das aplicacoes existentes (Mautic, Evolution, n8n).
...
#table(
  ...
  [Memoria RAM], [47.05 GB], [~3.06 GB], [1.5 GB], [APROVADO],
...
2. *Namespace de Volumes Isolados:* Todos os dados persistentes sao armazenados em volumes Docker dedicados (`zitadel_*`), impedindo sobrescrita de bancos de dados legados.
...
2. Navegue em *Stacks* > *+ Add stack* e defina o nome `zitadel`.
...
- *Tipo:* HTTP(s) Monitor.
- *URL Principal:* `https://sso.vpsconexao.org`.
...
1. *Via Portainer:* Selecione a stack `zitadel` e clique em *Delete this stack*.
2. *Via Terminal SSH:*
```bash
