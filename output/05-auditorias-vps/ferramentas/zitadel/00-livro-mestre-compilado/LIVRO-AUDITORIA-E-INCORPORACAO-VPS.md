...
**Alvo:** Zitadel Identity Management  
...
**Alvo de Incorporação:** Zitadel Identity Management  
**Data da Auditoria:** 28/08/2026  
**Veredito Técnico:** **TOTALMENTE VIAVEL (100% HOMOLOGADO)** (Score: 100/100)  
...
| :--- | :--- | :--- | :--- |
| **Sso Service** | Componente da Stack Zitadel Identity Management | `https://sso.vpsconexao.org` | Roteamento Traefik SNI |
...
**Alvo:** Zitadel Identity Management | **Data:** 28/08/2026
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `zitadel_*`).
  ...
  zitadel_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.zitadel.rule=Host(`sso.vpsconexao.org`)"
        - "traefik.http.routers.zitadel.entrypoints=websecure"
        - "traefik.http.routers.zitadel.tls=true"
        - "traefik.http.routers.zitadel.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.zitadel.loadbalancer.server.port=80"
...
5. No campo **Name**, digite exatamente: `zitadel`.
...
2. **Nome:** `Zitadel Identity Management - App Principal`.
3. **URL:** `https://sso.vpsconexao.org`.
...
Todos os recursos criados para o alvo `Zitadel Identity Management` foram encapsulados no namespace `zitadel`.
...
3. Localize a stack `zitadel` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=zitadel -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=zitadel_ -q | xargs -r docker volume rm
```
...
2. `docker stack ls` -> Confirme que a stack `zitadel` foi removida.
