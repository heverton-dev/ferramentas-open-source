...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[AnythingLLM Enterprise]\n
      #v(0.4em)
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** AnythingLLM Enterprise
...
#table(
  ...
  [Ai Service],
  [Componente da Stack AnythingLLM Enterprise],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `anything-llm_*`).
...
+ No campo **Name**, digite exatamente: `anything-llm`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `AnythingLLM Enterprise - App Principal`.
+ **URL:** `https://ai.vpsconexao.org`.
...
+ Localize a stack `anything-llm` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `anything-llm` foi removida.
