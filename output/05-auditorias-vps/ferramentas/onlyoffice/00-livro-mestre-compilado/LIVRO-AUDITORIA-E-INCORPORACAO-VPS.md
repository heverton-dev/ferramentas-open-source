...
5. No campo **Name**, digite exatamente: `onlyoffice`.
...
2. **Nome:** `ONLYOFFICE Document Server - App Principal`.
3. **URL:** `https://office.vpsconexao.org`.
...
3. Localize a stack `onlyoffice` e marque a caixa de sele??o ao lado dela.
...
2. `docker stack ls` -> Confirme que a stack `onlyoffice` foi removida.
