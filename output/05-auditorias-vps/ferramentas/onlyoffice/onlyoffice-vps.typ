...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[ONLYOFFICE Document Server]\n
      #v(0.4em)
...
**Alvo de Incorporacao:** ONLYOFFICE Document Server
...
#table(
  ...
  [Office Service],
  [Componente da Stack ONLYOFFICE Document Server],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `onlyoffice_*`).
  ...
  onlyoffice_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.onlyoffice.rule=Host(`office.vpsconexao.org`)"
        - "traefik.http.routers.onlyoffice.entrypoints=websecure"
        - "traefik.http.routers.onlyoffice.tls=true"
        - "traefik.http.routers.onlyoffice.tls.certresolver=letsencryptresolver"
...
+ No campo **Name**, digite exatamente: `onlyoffice`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `ONLYOFFICE Document Server - App Principal`.
+ **URL:** `https://office.vpsconexao.org`.
...
Todos os recursos criados para o alvo `ONLYOFFICE Document Server` foram encapsulados no namespace `onlyoffice`.
...
+ Localize a stack `onlyoffice` e marque a caixa de sele??o ao lado dela.
...
docker volume ls --filter name=onlyoffice -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=onlyoffice_ -q | xargs -r docker volume rm
```
...
+ `docker stack ls` -> Confirme que a stack `onlyoffice` foi removida.
