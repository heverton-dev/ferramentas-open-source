...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `podcastfy_*`).
...
| :--- | :--- | :--- | :--- |
| `audio.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
...
5. No campo **Name**, digite exatamente: `podcastfy`.
...
2. **Nome:** `Podcastfy Studio - App Principal`.
3. **URL:** `https://audio.vpsconexao.org`.
...
3. Localize a stack `podcastfy` e marque a caixa de seleção ao lado dela.
...
2. `docker stack ls` -> Confirme que a stack `podcastfy` foi removida.
