...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Podcastfy Studio]\n
      #v(0.4em)
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Podcastfy Studio
...
#table(
  ...
  [Audio Service],
  [Componente da Stack Podcastfy Studio],
  [`https://audio.vpsconexao.org`],
...
**Alvo:** Podcastfy Studio | **Data:** 28/08/2026
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `podcastfy_*`).
  ...
  podcastfy_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.podcastfy.rule=Host(`audio.vpsconexao.org`)"
        - "traefik.http.routers.podcastfy.entrypoints=websecure"
        - "traefik.http.routers.podcastfy.tls=true"
        - "traefik.http.routers.podcastfy.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.podcastfy.loadbalancer.server.port=80"
        - "traefik.http.services.podcastfy.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '1.5'
          memory: 1536M
...
- Registro A: `audio.vpsconexao.org` -> IP da VPS
...
+ No campo **Name**, digite exatamente: `podcastfy`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Podcastfy Studio - App Principal`.
+ **URL:** `https://audio.vpsconexao.org`.
...
Todos os recursos criados para o alvo `Podcastfy Studio` foram encapsulados no namespace `podcastfy`.
...
+ Localize a stack `podcastfy` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=podcastfy -q | xargs -r docker volume rm
```
...
+ `docker stack ls` -> Confirme que a stack `podcastfy` foi removida.
