...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Nextcloud Hub
...
#table(
  ...
  [Drive Service],
  [Componente da Stack Nextcloud Hub],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `nextcloud_*`).
...
+ No campo **Name**, digite exatamente: `nextcloud`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Nextcloud Hub - App Principal`.
...
+ Localize a stack `nextcloud` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `nextcloud` foi removida.
