...
A incorporacao da stack `nextcloud` demanda **2.0 vCPUs** e **2.5 GB de RAM**, mantendo margem de seguranca estrita.
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `nextcloud_*`).
...
3. Nomeie a stack como: `nextcloud`.
...
### Via Portainer:
1. Acesse **Stacks**, selecione `nextcloud` e clique em **Delete this stack**.
...
2. `docker stack ls` -> Confirmar ausencia da stack `nextcloud`.
