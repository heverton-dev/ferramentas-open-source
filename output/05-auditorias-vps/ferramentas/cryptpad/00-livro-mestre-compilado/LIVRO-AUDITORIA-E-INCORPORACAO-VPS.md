...
5. No campo **Name**, digite exatamente: `cryptpad`.
...
2. **Nome:** `CryptPad - App Principal`.
3. **URL:** `https://docs.vpsconexao.org`.
...
3. Localize a stack `cryptpad` e marque a caixa de seleção ao lado dela.
...
2. `docker stack ls` -> Confirme que a stack `cryptpad` foi removida.
