...
A incorporacao da stack `cryptpad` demanda **1.5 vCPUs** e **1.0 GB de RAM**, mantendo margem de seguranca estrita.
...
| **Memoria RAM Global** | 47.05 GB | ~3.06 GB | 1.0 GB | **~43.0 GB Livres** | APROVADO |
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `cryptpad_*`).
...
3. Nomeie a stack como: `cryptpad`.
...
2. **URL:** `https://docs.vpsconexao.org`.
...
### Via Portainer:
1. Acesse **Stacks**, selecione `cryptpad` e clique em **Delete this stack**.
...
2. `docker stack ls` -> Confirmar ausencia da stack `cryptpad`.
