...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[CryptPad]\n
      #v(0.4em)
...
A VPS de producao possui *12 vCPUs* e *47.05 GB de memoria RAM*, operando com folga substancial (*~43.99 GB de memoria livre*). A incorporacao do alvo demanda *1.5 vCPUs* e *1.0 GB de RAM*, preservando a estabilidade das aplicacoes existentes (Mautic, Evolution, n8n).
...
#table(
  ...
  [Processamento], [12 vCPUs], [~1.5 vCPUs], [1.5 vCPUs], [APROVADO],
...
2. *Namespace de Volumes Isolados:* Todos os dados persistentes sao armazenados em volumes Docker dedicados (`cryptpad_*`), impedindo sobrescrita de bancos de dados legados.
...
2. Navegue em *Stacks* > *+ Add stack* e defina o nome `cryptpad`.
  ...
  cryptpad_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.cryptpad.rule=Host(`docs.vpsconexao.org`)"
        - "traefik.http.routers.cryptpad.entrypoints=websecure"
        - "traefik.http.routers.cryptpad.tls=true"
        - "traefik.http.routers.cryptpad.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.cryptpad.loadbalancer.server.port=3000"
        - "traefik.http.services.cryptpad.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '1.5'
          memory: 1024M
...
- *Tipo:* HTTP(s) Monitor.
- *URL Principal:* `https://docs.vpsconexao.org`.
...
1. *Via Portainer:* Selecione a stack `cryptpad` e clique em *Delete this stack*.
2. *Via Terminal SSH:*
```bash
...
```bash
docker volume ls --filter name=cryptpad_ -q | xargs -r docker volume rm
```
