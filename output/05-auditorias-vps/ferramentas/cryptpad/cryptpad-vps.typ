...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `cryptpad_*`).
...
+ No campo **Name**, digite exatamente: `cryptpad`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `CryptPad - App Principal`.
+ **URL:** `https://docs.vpsconexao.org`.
...
+ Localize a stack `cryptpad` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `cryptpad` foi removida.
