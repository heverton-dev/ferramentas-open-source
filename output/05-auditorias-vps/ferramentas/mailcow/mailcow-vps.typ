...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Mailcow Dockerized]\n
      #v(0.4em)
...
- Memoria RAM abundante: 43.99 GB livres para suportar a carga de 4.0 GB com alta folga.
...
#table(
  ...
  [Mail Service],
  [Componente da Stack Mailcow Dockerized],
...
+ No campo **Name**, digite exatamente: `mailcow`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Mailcow Dockerized - App Principal`.
...
+ Localize a stack `mailcow` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `mailcow` foi removida.
