...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Open Notebook]\n
      #v(0.4em)
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Open Notebook
...
#table(
  ...
  [Notes Service],
  [Componente da Stack Open Notebook],
...
+ No campo **Name**, digite exatamente: `open-notebook`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Open Notebook - App Principal`.
+ **URL:** `https://notes.vpsconexao.org`.
...
+ Localize a stack `open-notebook` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `open-notebook` foi removida.
