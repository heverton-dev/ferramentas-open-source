...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Block Buzz Messaging Platform]\n
      #v(0.4em)
...
#table(
  ...
  [Buzz Service],
  [Componente da Stack Block Buzz Messaging Platform],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `buzz_*`).
...
+ No campo **Name**, digite exatamente: `buzz`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Block Buzz Messaging Platform - App Principal`.
...
+ Localize a stack `buzz` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `buzz` foi removida.
