...
**Alvo de Incorporacao:** RAGFlow DeepDoc Engine  
**Data da Auditoria:** 28/08/2026  
**Veredito Tecnico:** **TOTALMENTE VIAVEL (100% HOMOLOGADO)** (Score: 100/100)  
...
| **Memoria RAM Global** | 47.05 GB | ~3.06 GB | **~43.99 GB** | 3.0 GB | APROVADO |
...
| :--- | :--- | :--- | :--- |
| **Rag Service** | Componente da Stack RAGFlow DeepDoc Engine | `https://rag.vpsconexao.org` | Roteamento Traefik SNI |
...
**Alvo:** RAGFlow DeepDoc Engine | **Data:** 28/08/2026
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ragflow_*`).
  ...
  ragflow_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.ragflow.rule=Host(`rag.vpsconexao.org`)"
        - "traefik.http.routers.ragflow.entrypoints=websecure"
        - "traefik.http.routers.ragflow.tls=true"
        - "traefik.http.routers.ragflow.tls.certresolver=letsencryptresolver"
        - "traefik.http.services.ragflow.loadbalancer.server.port=80"
        - "traefik.http.services.ragflow.loadbalancer.passHostHeader=true"
      resources:
        limits:
          cpus: '2.0'
          memory: 3072M
...
| :--- | :--- | :--- | :--- |
| `rag.vpsconexao.org` | A | IP da VPS | DNS Only (Nuvem Cinza inicial) |
...
- Registro A: `rag.vpsconexao.org` -> IP da VPS
...
5. No campo **Name**, digite exatamente: `ragflow`.
...
2. **Nome:** `RAGFlow DeepDoc Engine - App Principal`.
3. **URL:** `https://rag.vpsconexao.org`.
...
Todos os recursos criados para o alvo `RAGFlow DeepDoc Engine` foram encapsulados no namespace `ragflow`.
...
3. Localize a stack `ragflow` e marque a caixa de sele??o ao lado dela.
...
docker volume ls --filter name=ragflow -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=ragflow_ -q | xargs -r docker volume rm
```
...
2. `docker stack ls` -> Confirme que a stack `ragflow` foi removida.
