...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[RAGFlow DeepDoc Engine]\n
      #v(0.4em)
...
#table(
  ...
  [Rag Service],
  [Componente da Stack RAGFlow DeepDoc Engine],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `ragflow_*`).
...
+ No campo **Name**, digite exatamente: `ragflow`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `RAGFlow DeepDoc Engine - App Principal`.
+ **URL:** `https://rag.vpsconexao.org`.
...
+ Localize a stack `ragflow` e marque a caixa de seleção ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `ragflow` foi removida.
