...
**Alvo de Incorporacao:** Chatwoot Omnichannel
...
#table(
  ...
  [Chat Service],
  [Componente da Stack Chatwoot Omnichannel],
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `chatwoot_*`).
...
+ No campo **Name**, digite exatamente: `chatwoot`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Chatwoot Omnichannel - App Principal`.
...
+ Localize a stack `chatwoot` e marque a caixa de sele??o ao lado dela.
...
+ `docker stack ls` -> Confirme que a stack `chatwoot` foi removida.
