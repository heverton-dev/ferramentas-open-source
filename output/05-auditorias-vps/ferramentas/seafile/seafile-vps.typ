...
#align(center)[
  #block(
    ...
    [
      ...
      #v(0.4em)
      #text(size: 20pt, fill: rgb("ffffff"), weight: "bold")[Seafile Professional/Community]\n
      #v(0.4em)
...
#v(1em)
**Alvo:** Seafile Professional/Community
...
#pagebreak()
= PARTE I · GUIAS EXECUTIVOS & VIABILIDADE ESTRATÉGICA
**Alvo de Incorporação:** Seafile Professional/Community
...
#table(
  ...
  [Drive Service],
  [Componente da Stack Seafile Professional/Community],
...
**Alvo:** Seafile Professional/Community | **Data:** 28/08/2026
...
+ **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `seafile_*`).
  ...
  seafile_app:
    ...
    deploy:
      ...
      labels:
        ...
        - "traefik.http.routers.seafile.rule=Host(`drive.vpsconexao.org`)"
...
+ No campo **Name**, digite exatamente: `seafile`.
...
+ **Tipo de Monitor:** HTTP(s).
+ **Nome:** `Seafile Professional/Community - App Principal`.
...
Todos os recursos criados para o alvo `Seafile Professional/Community` foram encapsulados no namespace `seafile`.
...
+ Localize a stack `seafile` e marque a caixa de seleção ao lado dela.
...
docker volume ls --filter name=seafile -q | xargs -r docker volume rm
```
...
```bash
docker volume ls --filter name=seafile_ -q | xargs -r docker volume rm
```
...
+ `docker stack ls` -> Confirme que a stack `seafile` foi removida.
