...
| :--- | :--- | :--- | :--- |
| **Drive Service** | Componente da Stack Seafile Professional/Community | `https://drive.vpsconexao.org` | Roteamento Traefik SNI |
...
2. **Namespace de Volumes Isolados:** Todos os volumes utilizam prefixos exclusivos (`workspace_*` ou `seafile_*`).
...
5. No campo **Name**, digite exatamente: `seafile`.
...
2. **Nome:** `Seafile Professional/Community - App Principal`.
...
3. Localize a stack `seafile` e marque a caixa de seleção ao lado dela.
...
2. `docker stack ls` -> Confirme que a stack `seafile` foi removida.
