...
- **R5 (Padrão Dossiê Executivo Diamante):** artefatos visuais seguem o padrão obrigatório: (1) Header com Hero Stats Bar e busca interativa client-side; (2) Título H1 e Deck justificados (`text-align: justify; text-justify: inter-word;`); (3) Tabela de dados 100% fluida sem scroll horizontal; (4) Cards verticais de largura total divididos em 4 seções padronizadas: *O Que Faz & Como Funciona* (com bloco de código e botão Copiar), *Análise Econômica* (SaaS substituídos e ROI), *Requisitos de Infraestrutura & Veredito* (com link do repositório) e *Como Usar no Dia a Dia* (grid com 3 mini-cards visuais de passos práticos). Proibido o uso de layouts em 2 colunas espremidas (`div.cols`).
