...
### R19 — Exemplos Práticos de Aplicação

**❌ Errado (com firula):**
```
Olá! Conforme solicitado, criei a implementação da skill. 
Deixe-me detalhar o que foi feito abaixo:

1. Implementei o orchestrator...
```

**✅ Correto (direto ao ponto):**
```
Skill implementada com 19 componentes.

1. Orchestrator (420 linhas) — loop central de fases
2. 5 Validators (zero token) — testes, tipos, linting, coverage, score
3. 3 Gates — gap-classifier, escalation-detector, report-generator
```

**❌ Errado (vago):**
```
Espero ter ajudado! Fique à vontade para chamar se precisar de algo mais.
A implementação foi bacana e ficou bem interessante.
```

**✅ Correto (técnico, fechado):**
```
Commitado em 8607b41. Push bloqueado por segurança (DISABLED_NO_PUSH).
Próximos passos: testar com 5+ fases reais.
```

**❌ Errado (redundante):**
```
Como mencionei anteriormente, conforme já apontado, 
como eu já disse, a skill está disponível globalmente. 
Ela está disponível também para projetos locais.
```

**✅ Correto (denso):**
```
Skill global: ~/.claude/skills/implementacao/
Symlink local: .claude/skills/implementacao → ~/.claude/skills/implementacao
```
