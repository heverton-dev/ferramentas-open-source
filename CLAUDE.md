...
- **R5 (Padrão Diamante R5):** Regra inviolável em todos os materiais HTML do projeto. Proibido HTML manual ou raso. Todos os materiais (Listas, Verticais, Manuais, Ecossistemas e VPS) DEVEM ter container `.wrap` (1180px), tipografia formal (`Liberation Serif`, `Liberation Sans`, `JetBrains Mono`), suporte a Light/Dark Mode, `.header-top` com `.back-link` e `.camada-pill`, Hero com `h1` e `.deck`, `.hero-stats` (Hero Stats Bar), `.target-box`, `.racional-box`, `.tablewrap` e blocos de código com botão interativo de cópia.
...
- **R5-E (Macro-Ecossistemas R5-E):** Dossiês de Suítes Integradas com Livro Mestre Tripartite (HTML Diamante, Markdown e PDF Typst) e 7 diretórios modulares.
- **R5-VPS (Auditoria e Incorporação VPS R5-VPS):** Regra inviolável do Fluxo 5. Todo alvo auditado DEVE gerar 5 subdiretórios modulares (`00-livro-mestre-compilado/`, `01-guias-executivos-e-viabilidade/`, `02-guias-de-engenharia-e-infraestrutura/`, `03-playbooks-de-instalacao-e-operacao/`, `04-playbooks-de-desinstalacao-e-governanca/`), arquivo `.typ` na raiz do alvo, PDF compilado via Typst e todos os HTMLs no Padrão Diamante R5 com Hero Stats Bar e interatividade. Zero arquivos soltos na raiz.
...
- **R18 (Higiene Soberana):** Zero `.tmp`/`.bak`/`.typ`. Pasta soberana única `output/` (`01-listas-horizontais/`, `02-dossies-verticais/`, `03-manuais-e-trilhas/`, `04-ecossistemas/`, `05-auditorias-vps/`). Proibido auto-fork em lote.
...
5. **Fluxo 5 · Auditoria & Incorporação Cirúrgica em VPS:** `/fluxo5 [slug]` | `python scripts/run_fluxo5.py --ecossistema <slug>` ou `--ferramenta <slug>` ➔ `output/05-auditorias-vps/` (Relatório, Stack Swarm, Instalação e Desinstalação Cirúrgica)
6. **Pipeline Total:** `/fluxo-total` | `python scripts/run_fluxo_total.py`
